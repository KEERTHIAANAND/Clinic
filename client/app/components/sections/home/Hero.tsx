...
const Hero: React.FC = () => {
  ...
  useEffect(() => {
    document.body.style.overflow = isAppointmentOpen ? "hidden" : "";
    document.documentElement.style.overflow = isAppointmentOpen ? "hidden" : "";
  ...
  return (
    <>
            ...
            <div className="space-y-10 max-w-2xl">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-8">
                <button
                  ...
                  aria-controls="home-appointment-modal"
                  aria-expanded={isAppointmentOpen}
                >
      ...
      {isAppointmentOpen && (
        <div
          id="home-appointment-modal"
          role="dialog"
          aria-modal="true"
          aria-label="Book free consultation"
          className="fixed inset-0 z-[220] bg-slate-950/65 backdrop-blur-sm p-4 md:p-8 overflow-y-auto"
          onClick={() => setIsAppointmentOpen(false)}
        >
          <div
            className="mx-auto mt-10 md:mt-14 w-full max-w-6xl flex justify-center"
            ...
            <div className="w-full">
              <div className="mb-4 flex justify-end">
                <button
                  type="button"
                  onClick={() => setIsAppointmentOpen(false)}
                  className="inline-flex items-center justify-center rounded-full bg-white p-3 text-slate-900 shadow-xl transition-colors hover:bg-slate-100"
                  aria-label="Close appointment form"
                >
                  <X size={20} />
                </button>
              </div>
              <AppointmentForm className="mx-auto w-full max-w-3xl" />
            </div>
          </div>
        </div>
      )}
